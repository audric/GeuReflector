...
A separate stack runs `test_routable.py` (2 tests) against a dedicated routable-prefix topology (defined in `topology.py`). See [Routable Prefixes](#routable-prefixes) below.
...
| `test_routable.py` | Routable-prefix tests (2 cases): explicit multi-hop transit (forward + return) and `*` wildcard with blacklist veto. Runs on a dedicated compose stack; configs and `docker-compose.routable.yml` are generated by `python3 generate_configs.py` (routable topology defined in `topology.py` under `ROUTABLE_REFLECTORS`), then started via `docker compose -f docker-compose.routable.yml up -d --build` and exercised with `python3 -m pytest test_routable.py`. |
